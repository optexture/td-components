...
class ExternalFiles:
	...
	def __init__(self, my_op):
		...
		self.Defaultcolor = self.my_op.pars('Defaultcolor*')
		self.Op_finder = self.my_op.op('opfind1')
		self.Extension_flag = self.my_op.par.Extensionflag.val
	...
	def Prompt_to_save(self, current_loc):
		...
		ext_color = self.my_op.pars("Extcolor*")
		...
		if current_loc == '/':
			...
			# in this case we are not external, so let's ask if we want to externalize the file
				...
				# the parent is not external, so let's ask about externalizing the tox
					...
					save_ext = ui.messageBox(
						sav_msg_box_title, sav_msg_box_msg + '\ncomponent: ' + current_loc.path,
	...
	def Save_tox(self, current_loc):
		...
		if not save_loc:
			self.Logtotextport('No file location selected for {}'.format(current_loc))
			return
	...
	def Flash_bg(self, parColors):
		...
		over_ride_color = self.my_op.pars(par_color)
